...
export default async function CheckoutPage() { const supabase = createClient(); const { data: { user } } = await supabase.auth.getUser(); if (!user) redirect('/login?next=/checkout'); const [{ data: addresses }, settings] = await Promise.all([supabase.from('addresses').select('*').eq('user_id', user.id).order('principal', { ascending: false }).order('created_at', { ascending: false }), getStoreSettings()]); return <main><Header settings={settings}/><div className="pt-[78px]"><CheckoutProgress/><CheckoutClient initialAddresses={addresses || []}/></div><Footer settings={settings}/><WhatsAppButton settings={settings}/></main>; }
