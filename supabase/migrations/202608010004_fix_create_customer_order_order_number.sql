-- Corrige a ambiguidade entre a coluna orders.order_number e o campo retornado pela função.
...
declare
  ...
  v_subtotal bigint := 0; v_created_order_id uuid; v_created_order_number bigint;
begin
  ...
  if not exists (select 1 from public.addresses a where a.id = p_address_id and a.user_id = v_customer_id) then raise exception 'O endereço selecionado não pertence à sua conta.' using errcode = '42501'; end if;

  insert into public.orders as o (customer_id, address_id, payment_method, notes)
  values (v_customer_id, p_address_id, p_payment_method, nullif(trim(coalesce(p_notes, '')), ''))
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    select p.* into v_product from public.products p where p.id = v_product_id and p.active = true for share;
    ...
    insert into public.order_items as oi (order_id, product_id, product_name, size, color, unit_price, quantity, subtotal)
    values (v_created_order_id, v_product.id, v_product.name, v_size, v_color, v_product.price_cents, v_quantity, v_product.price_cents::bigint * v_quantity);
    v_subtotal := v_subtotal + (v_product.price_cents::bigint * v_quantity);
  end loop;

  update public.orders as o set subtotal = v_subtotal, shipping = 0, discount = 0, total = v_subtotal where o.id = v_created_order_id;
  return query select v_created_order_id as order_id, v_created_order_number as order_number;
end;
$$;
