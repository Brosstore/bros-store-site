-- Substitui a criação de pedido para baixar o estoque na mesma transação.
...
declare
  ...
  v_product_id uuid; v_quantity integer; v_size text; v_color text; v_subtotal bigint := 0; v_created_order_id uuid; v_created_order_number bigint; v_before integer;
begin
  ...
  insert into public.orders as o (customer_id, address_id, payment_method, notes) values (v_customer_id, p_address_id, p_payment_method, nullif(trim(coalesce(p_notes, '')), '')) returning o.id, o.order_number into v_created_order_id, v_created_order_number;
