...
function credentialEnvironment(value, name) {
...
function resolvePublicKey(env) {
  ...
  if (current && legacy && current !== legacy) {
    throw new MercadoPagoConfigurationError(
      'As chaves públicas atual e legada do Mercado Pago são diferentes.',
      'CONFLICTING_PUBLIC_KEYS',
    );
  }
  if (!current && !legacy) {
    throw new MercadoPagoConfigurationError(
...
function resolveEnvironment(env, accessToken, publicKey) {
  const tokenEnvironment = credentialEnvironment(accessToken, 'MERCADO_PAGO_ACCESS_TOKEN');
  const keyEnvironment = credentialEnvironment(publicKey, 'NEXT_PUBLIC_MERCADO_PAGO_PUBLIC_KEY');
  if (tokenEnvironment !== keyEnvironment) {
    throw new MercadoPagoConfigurationError(
      'Credenciais do Mercado Pago misturam os ambientes de teste e produção.',
  ...
  if (configured && configured !== tokenEnvironment) {
    throw new MercadoPagoConfigurationError(
      'MERCADO_PAGO_ENVIRONMENT não corresponde às credenciais configuradas.',
      'MIXED_CREDENTIAL_ENVIRONMENTS',
    );
  }
  return configured || tokenEnvironment;
}
