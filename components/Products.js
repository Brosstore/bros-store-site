...
export default function Products({ products }) {
  return <section id="destaques" className="bg-[#0e0e0e]"><div className="section">
    ...
    {products?.length ? <div className="grid gap-5 min-[420px]:grid-cols-2 lg:grid-cols-4">{products.map((product) => <ProductCard product={product} key={product.id}/>)}</div> : <p className="rounded-2xl border border-dashed border-white/15 p-10 text-center text-zinc-400">Novos destaques serão adicionados em breve.</p>}
  </div></section>;
}
