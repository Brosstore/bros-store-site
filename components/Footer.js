...
export default function Footer() {
  ...
  return <footer className="border-t border-white/10 bg-ink">
    <div className="section grid gap-12 py-16 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
      <div><a href="#inicio" className="text-2xl font-extrabold tracking-[.08em]">BROS<span className="ml-1 text-brand">STORE</span></a><p className="mt-5 max-w-[12rem] text-base font-bold text-white">{siteConfig.tagline}</p><p className="mt-2 text-sm text-zinc-500">Moda • Streetwear • Acessórios</p></div>
      <div><h3 className="font-mono text-[10px] font-bold uppercase tracking-[.2em] text-brand">Categorias</h3><div className="mt-5 grid gap-3 text-sm text-zinc-400"><a className="transition hover:text-brand" href="#categorias">Camisetas</a><a className="transition hover:text-brand" href="#categorias">Calçados</a><a className="transition hover:text-brand" href="#categorias">Bonés</a><a className="transition hover:text-brand" href="#categorias">Acessórios</a></div></div>
